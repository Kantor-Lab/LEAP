...
void BridgeROS2::ros_node_thread_main(Yaml cfg)
{
  ...
  try
  {
    ...
    if (rosNode_->get_parameter("use_sim_time").as_bool())
    {
      mrpt::Clock::setActiveClock(mrpt::Clock::Source::Simulated);
      
      // Subscribe to /clock and feed it into mrpt::Clock
      subClock_ = rosNode_->create_subscription<rosgraph_msgs::msg::Clock>(
          "/clock", rclcpp::ClockQoS(),
          [](const rosgraph_msgs::msg::Clock& msg)
          {
            mrpt::Clock::setSimulatedTime(
    ...
    // TF buffer:
    tf_buffer_ = std::make_shared<tf2_ros::Buffer>(ros_clock_);  // ros_clock_
    ...
    mrpt::poses::CPose3D& des, const std::string& frame, const std::string& referenceFrame)
{
  try
  {
    geometry_msgs::msg::TransformStamped ref_to_trgFrame =
        tf_buffer_->lookupTransform(referenceFrame, frame, rclcpp::Time(0));
